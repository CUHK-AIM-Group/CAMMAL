...
def ner_init(compute,gpu_id):
    pipe = load_model(compute,gpu_id)
    return pipe

def ner_all_prediction(corpus_list, compute, gpu_id):
    pipe = load_model(compute, gpu_id)
    output_list = []
    for corpus in tqdm(corpus_list):
        # print("corpus:",corpus, type(corpus))
        # start_time = time.time()
        out = ner_prediction(corpus, compute, pipe)
...
def ner_prediction(corpus, compute, pipe):
