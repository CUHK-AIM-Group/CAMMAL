...
class MultimodalPretrainingDataset(data.Dataset):
    def __init__(self, split="train", transform=None, data_pct=1.0,
        ...
        # self.filenames = self.filenames[:2048]
    ...
    def load_text_data(self, split):
        ...
        #         img_path = os.path.join(MIMIC_CXR_IMAGE_DIR, path.split('/')[-1])
        #         if not os.path.exists(img_path) or not os.path.getsize(img_path):
        #             not_found += 1
        #             not_found_list.append((path, img_path))
