...
class MultimodalPretrainingDataset(data.Dataset):
    def __init__(self, split="train", transform=None, data_pct=1.0,
        ...
        patch_token_path = './adamatch/data/topk_patch_list/top1000_2023_09_21_19_00_21_patches_list.pickle'
        ...
        for cxr_id in self.patch_token_dict:
            patches = self.patch_token_dict[cxr_id]['patches'] # num x ch x p_h x p_w 
            self.vq_code_list.extend(self.patch_token_dict[cxr_id]['vq_code'])
            # patches = torch.permute(patches, [0, 2, 3, 1]).numpy() # num x p_h x p_w x ch 
            ...
            for i in range(patches.shape[0]):
                ...
                patch_list.append(patches[i]) # ch x p_h x p_w 
        ...
        raw_patch_list = torch.zeros(fake_img_num, ch, input_size, input_size) #each_p_num*p_h, each_p_num*p_w)
        ...
        for img_idx in range(fake_img_num):
            ...
            for i in range(each_p_num):
                ...
                for j in range(each_p_num):
                    ...
                    row_start = (i*p_h) + pad_h
                    row_end = (i+1)*p_h + pad_h
                    col_start = (j*p_w) + pad_w
                    col_end = (j+1)*p_w + pad_w
                    raw_patch_list[img_idx,:,row_start:row_end,col_start:col_end] = patch_list[idx] # ch x p_h x p_w 
                    idx += 1
        # reshape patch list : fake_img_num x ch x input_sizex input_size
        raw_patch_list = torch.permute(raw_patch_list, [0, 2, 3, 1]) # fake_img_num x input_size x input_size x ch
        self.patch_list = []
        for img_idx in range(fake_img_num):
            tmp = np.array(raw_patch_list[img_idx].numpy(), dtype=np.uint8)
            tmp_img_patch = Image.fromarray(tmp)
            transformed_img = self.transform(tmp_img_patch) # ch x input_size x input_size
            transformed_img = transformed_img.unsqueeze(0) # 1 x ch x input_size x input_size
            self.patch_list.append(transformed_img)
        self.patch_list = torch.cat(self.patch_list, dim=0)
        # print("self.vq_code_list[0]:----",self.vq_code_list[0])
        # print("self.vq_code_list:",len(self.vq_code_list))
