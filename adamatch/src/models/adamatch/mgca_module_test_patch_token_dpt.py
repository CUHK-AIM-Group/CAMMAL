...
from mgca.datasets.pretrain_dataset_test_patch_token_dpt import (MultimodalPretrainingDataset,
                                            multimodal_collate_fn)
from mgca.datasets.pretrain_dataset_test_patch_token_dpt_openI import MultimodalPretrainingDataset as MultimodalPretrainingDataset_openI
from mgca.datasets.pretrain_dataset_test_patch_token_dpt_openI import multimodal_collate_fn as multimodal_collate_fn_openI
from mgca.datasets.transforms import DataTransforms, TestDataTransforms
...
class MGCA(LightningModule):
    ...
    def forward(self, batch, batch_idx, split="train"):
        ...
        # print('batch["imgs"]:',type(batch["imgs"]))
        # print('batch["imgs"]:',batch["imgs"].shape)
        img_feat_q, patch_feat_q = self.img_encoder_q(
            batch["imgs"])
        # print('batch["imgs"]:',batch["imgs"])
        # print('batch["imgs"]:',batch["imgs"].shape,"img_feat_q:",img_feat_q.shape, "patch_feat_q:",patch_feat_q.shape)
        # torch.Size([102, 196, 768])
        ...
        bs_w, word_token_num, dim2  = word_emb_q.shape
        patch_emb_q = torch.reshape(patch_emb_q, (bs*patch_token_num, dim1))
        word_emb_q = torch.reshape(word_emb_q, (bs_w*word_token_num, dim2)).transpose(1,0)
        ...
        patch_sim_matrix = torch.reshape(tokens_sim_matrix, (bs*patch_token_num, bs_w, word_token_num)) # (bs x patch_token_num) x bs x word_token_num
        patch_sim_matrix = patch_sim_matrix.max(2).values # (bs x patch_token_num) x bs
        patch_sim_matrix = torch.reshape(patch_sim_matrix, [bs, patch_token_num, bs_w]) # bs x patch_token_num x bs
        patch_sim_matrix = patch_sim_matrix.mean(1) # bs x bs 
        # get word_sim_matrix
        word_sim_matrix = torch.reshape(tokens_sim_matrix, (bs, patch_token_num, bs_w*word_token_num)) # bs x patch_token_num x (bs x word_token_num)
        word_sim_matrix = word_sim_matrix.max(1).values # bs x (bs x word_token_num)
        word_sim_matrix = torch.reshape(word_sim_matrix, [bs, bs_w, word_token_num]) # bs x (bs x word_token_num)
        ...
        tokens_sim_matrix_reshape = torch.reshape(tokens_sim_matrix, [bs, patch_token_num, bs_w, word_token_num])
        tokens_sim_matrix_reshape, _ = tokens_sim_matrix_reshape.max(3) # bs, patch_token_num, bs_w

        # return loss_token, loss_token0, loss_token1, acc1, acc5, tokens_sim_matrix_reshape #patch_sim_matrix, word_sim_matrix
        return 0, 0, 0, 0, 0, tokens_sim_matrix_reshape #patch_sim_matrix, word_sim_matrix
    ...
    def test_step(self, batch, batch_idx):
        loss_token, loss_token0, loss_token1, acc1, acc5, \
                    tokens_sim_matrix = self(batch, batch_idx, "valid")
        ...
        device = str(tokens_sim_matrix.device) # (bs, patch_token_num, bs_w)
        ...
        item_list = []
        topk_patch = 10
        vq_code_list = batch['vq_code'] # (102*14*14, )
        # dim0 = len(vq_code_list)
        # dim1 = len(vq_code_list[0])
        # dim2 = len(vq_code_list[0][0])
        for i, (img_path, sentence, input_ids, special_token_mask) \
            in enumerate(tqdm(zip(batch['path'],batch['sentences'],batch['caption_ids'],batch['special_tokens_mask']), total=bs)):
            cxr_id = img_path.split('/')[-1].split('.')[0] # id
            # get topk patches for each report // # bs, patch_token_num, bs_w
            sim_matrix = tokens_sim_matrix[:,:,i]
            bs, patch_token_num = sim_matrix.shape
            sim_matrix = torch.reshape(sim_matrix, [bs*patch_token_num,]) # bs*patch_token_num x 1
            sorted_sim_matrix, sorted_indices = torch.sort(sim_matrix, descending=True) 
            topk_indices = sorted_indices[:topk_patch]
            topk_vq_code_list = findElements(vq_code_list, topk_indices)
            # print("topk_vq_code_list:",len(topk_vq_code_list),topk_vq_code_list)
            flatten_code_list = list(set(list(chain.from_iterable(topk_vq_code_list))))
            # print("topk_vq_code_list-set:",set(flatten_code_list))
            # reshape_sorted_indices = torch.reshape(sorted_indices, [bs,patch_token_num,])

            # topk_vq_code_list = []
            # count =0
            # for b in range(bs):
            #     for k in range(patch_token_num):
            #         if count >= topk_patch:
            #             break
            #         indice = reshape_sorted_indices[b,k]
            #         row = indice // dim1 - 1
            #         col = indice % dim1 - 1 
            #         print("indice:",indice, "row:",row,"col:",col)
            #         topk_vq_code_list.extend(vq_code_list[b][row][col])
            #         # print(vq_code_list[b][row][col])
            #         count +=1
            #     # for d1 in range(dim1):
            #     #     for d2 in range(dim2):
            #     #         reshape_sorted_indices[b,d1]
            # topk_vq_code_list = set(topk_vq_code_list)
            # print("topk_vq_code_list:",len(topk_vq_code_list),topk_vq_code_list,)

            content = {
                'path': img_path,
                'vq_code': flatten_code_list
                # 'sentence': sentence,
                ...
                # 'word_id': word_id.cpu().detach().numpy()
            }
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument("--result_path", type=str, default="./adamatch/data/results")
...
def cli_main():
    ...
    # save results path
    args.result_path = os.path.join(args.result_path, args.ckpt_path.split('/')[-2] + "_patches_" + args.split )
