...
from mgca.datasets.pretrain_dataset_test_token import (MultimodalPretrainingDataset,
                                            multimodal_collate_fn)
from mgca.datasets.pretrain_dataset_test_token_openI import MultimodalPretrainingDataset as MultimodalPretrainingDataset_openI
from mgca.datasets.pretrain_dataset_test_token_openI import multimodal_collate_fn as multimodal_collate_fn_openI
...
class MGCA(LightningModule):
    ...
    def __init__(self,
        ...
        if img_encoder == 'dpt_medium':
            ...
            # patch_embeds = [self.img_encoder_q.patch_embed2, self.img_encoder_q.patch_embed3, self.img_encoder_q.patch_embed4]
    ...
    def forward(self, batch, batch_idx, split="train"):
        ...
        print('batch["caption_ids"]:',batch["caption_ids"].shape)
        ...
        #                 self.img_encoder_q.patch_embed4.box_coder.boxes ]
        ...
        return 0, 0, 0, 0, 0, tokens_sim_matrix_reshape, boxes_info_list #patch_sim_matrix, word_sim_matrix
    ...
    def test_step(self, batch, batch_idx):
        loss_token, loss_token0, loss_token1, acc1, acc5, \
                    tokens_sim_matrix, boxes_info_list = self(batch, batch_idx, "valid")
        ...
        item_list = []
        # for i, (img_path, sentence, input_ids, special_token_mask , tokens_sim, word_id) \
        #     in enumerate(tqdm(zip(batch['path'],batch['sentences'],batch['caption_ids'],batch['special_tokens_mask'], \
        #                                                 tokens_sim_matrix, batch['word_ids']), total=bs)):
        for i, (img_path, sentence, input_ids, special_token_mask , tokens_sim, word_id) \
            in enumerate(zip(batch['path'],batch['sentences'],batch['caption_ids'],batch['special_tokens_mask'], \
                                                        tokens_sim_matrix, batch['word_ids'])):
            # get box 
            ...
            for id_st, (boxes_info, self.output_size) in enumerate(zip(boxes_info_list, self.output_sizes)):
                ...
                # torch_item.append({
                #     "image_name": img_path.replace('/root/CLS-LOC/', ''),
                #     "boxes": boxes,
                # })

            # boxes_list = torch.cat(boxes_list, dim=0)
            # centers_list = torch.cat(centers_list, dim=0)
            # anchors_list = torch.cat(anchors_list, dim=0)
            # print("boxes_list:",boxes_list.shape,"centers_list:",centers_list.shape,"anchors_list:",anchors_list.shape)
...
def cli_main():
    ...
    # save results path
    args.result_path = os.path.join(args.result_path, args.ckpt_path.split('/')[-2] + "_newkeywords_" + args.split )
    os.makedirs(args.result_path, exist_ok=True)

    # seed
    seed_everything(args.seed)
    # print("aaaaa----args.use_trainset:",args.use_trainset)
