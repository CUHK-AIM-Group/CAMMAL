...
class MGCA(LightningModule):
    ...
    def __init__(self,
        ...
        self.img_encoder_q.load_state_dict(dict(state_dict), strict=False)
        self.text_encoder_q.load_state_dict(dict(state_dict), strict=False)
    ...
    def forward(self, batch, batch_idx, split="train"):
        ...
        # patch_token_sim_list = []
        # word_token_sim_list = []
        '''patch_sim_matrix = torch.zeros((patch_emb_q.shape[0], word_emb_q.shape[0])) # bs x bs
        word_sim_matrix = torch.zeros((patch_emb_q.shape[0], word_emb_q.shape[0])) # bs x bs
        for i, (cap_len, word_emb) in enumerate(zip(batch["cap_lens"], word_emb_q)):
            word = word_emb[1:cap_len,:] # word_token_num x feat_dim
            for j, img_emb in enumerate(patch_emb_q):
                sim = img_emb @ word.transpose(1, 0) # img_token_num x word_token_num
                # similarity for patch and word tokens
                patch_token_sim = sim.max(1).values # img_token_num
                word_token_sim = sim.max(0).values # word_token_num
                # sort
                sorted_patch_sim, sorted_patch_sim_indices = torch.sort(patch_token_sim, 0, True)
                sorted_word_sim, sorted_word_sim_indices = torch.sort(word_token_sim, 0, True)
                # quarter of token similarities
                quarter_len_patch = sorted_patch_sim.shape[0] // 4
                quarter_len_word = sorted_word_sim.shape[0] // 4
                # get the top 25%
                # print("patch_token_sim:",patch_token_sim.shape,"word_token_sim:",word_token_sim.shape, "quarter_len_patch:",quarter_len_patch, "quarter_len_word:",quarter_len_word)
                patch_sim = sorted_patch_sim[:quarter_len_patch].mean()#.unsqueeze(0)
                word_sim = sorted_word_sim[:quarter_len_word].mean()#.unsqueeze(0)
                # patch_token_sim_list.append(patch_sim) # quarter_len_patch
                # word_token_sim_list.append(word_sim) # quarter_len_word
                patch_sim_matrix[i][j] = patch_sim
        ...
        # get top 20% token similarity
        patch_sim_matrix, sorted_patch_sim_indices = torch.sort(patch_sim_matrix, 2, True)
        del sorted_patch_sim_indices
        patch_sim_matrix = patch_sim_matrix[:,:, :int(word_token_num*0.25)]
        ...
        # get top 20% token similarity
        word_sim_matrix, sorted_word_sim_indices = torch.sort(word_sim_matrix, 1, True)
        del sorted_word_sim_indices
        word_sim_matrix = word_sim_matrix[:,:int(patch_token_num*0.25),:]
        ...
        # return loss_ita, loss_token, acc1, acc5
        # return loss_ita, loss_token, loss_token0, loss_token1, acc1, acc5
