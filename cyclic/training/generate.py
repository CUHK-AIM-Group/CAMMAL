...
from .consts import END_KEY, PROMPT_FOR_GENERATION_FORMAT, PROMPT_FOR_GENERATION_FORMAT_NOINPUT, RESPONSE_KEY, PROMPT_WITH_KEYWORDS_FOR_GENERATION_FORMAT, PROMPT_WITH_KEYPATCHES_FOR_GENERATION_FORMAT
from .mimiccxr_vq_dataset import get_extract_vq_fun, get_inject_vq_fun, get_extract_vq_fun_new, CXR_VQ_VQ_REPLACE_TEMPLATE, CXR_VQ_VQ_KEYPATCH_REPLACE_TEMPLATE, get_inject_vq_fun_keypatch
...
def load_model_tokenizer_for_generate_separate(
    ...
    Returns:
        Tuple[PreTrainedModel, PreTrainedTokenizer]: model and tokenizer
    """
    print("tokenizer_name_or_path:",tokenizer_name_or_path)
...
class InstructionTextGenerationPipeline(Pipeline):
    def __init__(
        self, *args, do_sample: bool = True, max_new_tokens: int = 256, top_p: float = 0.92, top_k: int = 0, use_keypatch=False, **kwargs
    ):
        ...
        Args:
            ...
            top_k (int, optional): The number of highest probability vocabulary tokens to keep for top-k-filtering.
                Defaults to 0.
        """
        self.use_keypatch = use_keypatch
    ...
    def preprocess(self, instruction_text, **generate_kwargs):
        input_len = len(instruction_text)
        if input_len == 2:
            instruction_text, input_text = instruction_text
        else:
            instruction_text, input_text, keycodes = instruction_text
        ...
        elif type(input_text) is str: # text2image
            if input_len == 2:
                ...
                prompt_text = PROMPT_WITH_KEYPATCHES_FOR_GENERATION_FORMAT.format(instruction=instruction_text, input=input_text, keypatch=CXR_VQ_VQ_KEYPATCH_REPLACE_TEMPLATE)
                ...
                inputs["input_ids"] = torch.tensor(get_inject_vq_fun_keypatch(self.tokenizer)(inputs["input_ids"][0].numpy().tolist(), keycodes))[None, ...]
                inputs["attention_mask"] = torch.ones_like(inputs["input_ids"])

        elif type(input_text) is list: # image2text
            if input_len == 2:
                prompt_text = PROMPT_FOR_GENERATION_FORMAT.format(instruction=instruction_text, input=CXR_VQ_VQ_REPLACE_TEMPLATE)
            else:
                prompt_text = PROMPT_WITH_KEYWORDS_FOR_GENERATION_FORMAT.format(instruction=instruction_text, input=CXR_VQ_VQ_REPLACE_TEMPLATE, keyword=keycodes)
            # print("prompt_text:",prompt_text)
            inputs = self.tokenizer(
                prompt_text,
                return_tensors="pt",
    ...
    def _forward(self, model_inputs, **generate_kwargs):
        ...
        vq = get_extract_vq_fun_new(self.tokenizer, use_keypatch=self.use_keypatch)(generated_sequence)
